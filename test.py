...
async def test_tiktok_download(temp_content_dir):
    with (
        patch("pyktok.alt_get_tiktok_json") as mock_get_json,
        patch("pyktok.save_tiktok") as mock_save,
    ):
        mock_get_json.return_value = {
            "__DEFAULT_SCOPE__": {
                "webapp.video-detail": {"shareMeta": {"desc": "Test #description"}}
            }
        }
...
async def test_instagram_download(temp_content_dir):
    with (
        patch("instaloader.Instaloader") as mock_loader,
...
def test_tiktok_download_error():
    with patch("pyktok.alt_get_tiktok_json") as mock_get_json:
        ...
        with pytest.raises(
            ValueError, match="Ошибка при загрузке TikTok видео: API Error"
        ):
...
def test_youtube_download_error():
    with patch("pytubefix.YouTube") as mock_yt:
        ...
        with pytest.raises(
            ValueError, match="Ошибка при загрузке YouTube видео: Video unavailable"
        ):
...
def test_instagram_download_error():
    with patch("instaloader.Post") as mock_post:
        ...
        with pytest.raises(
            ValueError, match="Ошибка при загрузке Instagram видео: Invalid shortcode"
        ):
...
def test_multiple_hashtags_removal():
    with (
        ...
        mock_get_json.return_value = {
            "__DEFAULT_SCOPE__": {
                "webapp.video-detail": {"shareMeta": {"desc": "#Test #video #tags"}}
            }
        }
        mock_save.return_value = None
